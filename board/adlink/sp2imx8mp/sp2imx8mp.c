...
int board_fix_fdt_extra(void *fdt)
{
	if (is_imx8mp()) {
		...
		//memcpy (buffer, gd->spl_handoff->arch.buffer, 768);
		debug("%s() arch.sku: %d\n", __func__, gd->spl_handoff->arch.sku);
		debug("%s() buffer: %s\n", __func__, gd->spl_handoff->arch.buffer);
		ret = parse_eeprom(gd->spl_handoff->arch.buffer, key, disp);
		debug("%s() disp: %s ret=%d\n", __func__, disp, ret);
...
int board_phys_sdram_size(phys_size_t *size)
{
	phys_size_t dram_size[] = {
		(phys_size_t)2 << 30, /* default */
		(phys_size_t)4 << 30, /* &dram_timing_lpddr4_4gb, */
		(phys_size_t)8 << 30, /* &dram_timing_lpddr4_8gb, */
		(phys_size_t)1 << 30, /* &dram_timing_lpddr4_1gb, */
		(phys_size_t)2 << 30, /* NULL, default to 2gb */
		(phys_size_t)2 << 30, /* NULL, default to 2gb */
		(phys_size_t)2 << 30, /* NULL, default to 2gb */
		(phys_size_t)2 << 30, /* NULL, default to 2gb */
	};
	uint32_t hwid = gd->spl_handoff->arch.sku;
	if (hwid >= 0 && hwid <= 8) {
		*size = dram_size[hwid];
	} else {
...
/*
 * mirrored from spl_board_boot_device() from spl.c
 * for setting env variable for boot_device
 */
#ifndef CONFIG_SPL_BUILD
char* board_boot_device(enum boot_device boot_dev)
{
	switch (boot_dev) {
	case SD1_BOOT:
	case SD2_BOOT:
