...
struct imx_ldb_priv {
	...
	enum ENUM_BITMAPPING data_mapping;
};
...
static int imx_ldb_set_backlight(struct udevice *dev, int percent)
{
	...
	debug("%s(): ldb: set backlight\n", __func__);

	if (priv->lvds1) {
		ret = panel_enable_backlight(priv->lvds1);
		if (ret) {
			debug("%s(): ldb: Cannot enable lvds1 backlight\n", __func__);
			return ret;
		}
	...
	if (priv->lvds2) {
		...
		if (ret) {
			debug("%s(): ldb: Cannot enable lvds2 backlight\n", __func__);
			return ret;
		}
...
static int imx_ldb_of_to_plat(struct udevice *dev)
{
	...
	if (!priv->lvds1 && !priv->lvds2) {
		debug("%s(): ldb: No remote panel for '%s' (ret=%d)\n", __func__,
		      dev_read_name(dev), ret);
		return ret;
	}

	debug("%s(): ldb: got panels: 0x%lx 0x%lx\n", __func__, priv->lvds1, priv->lvds2);
...
static int imx_ldb_check_timing(struct udevice *dev, struct display_timing *timing)
{
	struct imx_ldb_priv *priv = dev_get_priv(dev);
	struct display_timing timings;
	...
	if (priv->lvds2) {
		...
		debug("%s(): ldb: lvds2: ldb_ctrl: 0x%lx\n", __func__, ldb_ctrl);
	}
...
static int imx_ldb_attach(struct udevice *dev)
{
	...
	debug("%s(): ldb: attach data-width:%d data-mapping:%d\n", __func__, priv->data_width, priv->data_mapping);
...
static int imx_ldb_probe(struct udevice *dev)
{
	...
	if (ret || (priv->data_width != 18 && priv->data_width != 24)) {
		debug("%s(): data width not set or invalid, force default to 24\n", __func__);
		priv->data_width = 24;
	}
	/* fsl,data-mapping */
	priv->data_mapping = of_get_data_mapping(dev);
	if (priv->data_mapping < 0) {
		debug("%s(): data map not set or invalid, force default to spwg\n", __func__);
		priv->data_mapping = SPWG;
	}
...
struct video_bridge_ops imx_ldb_ops = {
	...
	.check_timing = imx_ldb_check_timing,
